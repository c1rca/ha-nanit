...
`;let yt=class extends ot{setConfig(t){this._config={...t}}_entityChanged(t,e){const i=e.detail.value||void 0;this._config&&i!==this._config[t]&&this._updateConfig({[t]:i})}_toggleChanged(t,e){const i=e.target.checked;this._updateConfig({[t]:i})}_updateConfig(t){const e={...this._config,...t};for(const[i,s]of Object.entries(t))void 0===s&&delete e[i];this._config=e,this.dispatchEvent(new CustomEvent("config-changed",{bubbles:!0,composed:!0,detail:{config:e}}))}render(){return this.hass&&this._config?D`
  ...
  `,t([pt({attribute:!1})],yt.prototype,"hass",void 0),t([ut()],yt.prototype,"_config",void 0),yt=t([lt("nanit-card-editor")],yt);let wt=class extends ot{constructor(){super(...arguments),this._streamLoaded=!1,this._showNetwork=!1,this._streamEpoch=0,this._lastVideoTime=0,this._sawProgress=!1,this._stallStrikes=0,this._healthyTicks=0,this._reloadAttempts=0,this._reloadCooldownUntil=0,this._watchedEpoch=-1}static getConfigElement(){return document.createElement("nanit-card-editor")}static getStubConfig(t){const e=Object.keys(t.states).find(e=>e.startsWith("camera.")&&"nanit"===t.entities[e]?.platform);return{type:"custom:nanit-card",camera_entity_id:e||""}}setConfig(t){if(!t)throw new Error("Invalid configuration");this._config=t}getCardSize(){return 5}_entities(){return this._config?.camera_entity_id&&this.hass?function(t,e){const i={};i.camera=e;const s=t.entities[e];if(s?.device_id){const e=s.device_id,r=[],n=[];for(const i of Object.keys(t.entities))t.entities[i].device_id===e&&("diagnostic"===t.entities[i].entity_category?n.push(i):r.push(i));vt(i,r,t),function(t,e,i){for(const s of e){const[e]=s.split(".",1),r=s.split(".")[1]??"",n=i.states[s]?.attributes.device_class;"sensor"===e&&("signal_strength"===n?t.wifi_signal=s:"frequency"===n?t.wifi_frequency=s:r.endsWith("_wifi_ssid")&&(t.wifi_ssid=s))}}(i,n,t)}else{const s=e.split(".")[1]??"";vt(i,Object.keys(t.states).filter(t=>t!==e&&t.split(".")[1]?.startsWith(s.split("_camera")[0]||s)),t)}return i}(this.hass,this._config.camera_entity_id):{}}_isCameraOn(t){return!t.power||"on"===this.hass.states[t.power]?.state}_fireMoreInfo(t){this.dispatchEvent(new CustomEvent("hass-more-info",{bubbles:!0,composed:!0,detail:{entityId:t}}))}_toggleService(t,e,i){this.hass.callService(t,e,{entity_id:i})}disconnectedCallback(){super.disconnectedCallback(),this._stopStreamWatch()}updated(t){super.updated(t);this.renderRoot.querySelector("ha-camera-stream")?(this._watchedEpoch!==this._streamEpoch&&(this._watchedEpoch=this._streamEpoch,this._lastVideoTime=0,this._sawProgress=!1,this._stallStrikes=0,this._healthyTicks=0),this._startStreamWatch()):this._stopStreamWatch()}_startStreamWatch(){this._streamWatchTimer||(this._streamWatchTimer=setInterval(()=>this._checkStreamHealth(),1e3))}_stopStreamWatch(){this._streamWatchTimer&&(clearInterval(this._streamWatchTimer),this._streamWatchTimer=void 0)}_findStreamVideo(t){const e=t.shadowRoot;if(!e)return null;const i=e.querySelector("video");if(i)return i;const s=e.querySelector("ha-hls-player, ha-web-rtc-player"),r=s?.shadowRoot?.querySelector("video");if(r)return r;for(const t of Array.from(e.querySelectorAll("*"))){const e=t.shadowRoot?.querySelector("video");if(e)return e}return null}_checkStreamHealth(){const t=this.renderRoot.querySelector("ha-camera-stream");if(!t)return;const e=this._findStreamVideo(t);if(!e)return void(t.shadowRoot?.querySelector("img, canvas")&&(this._streamLoaded||(this._streamLoaded=!0)));if(e.currentTime>this._lastVideoTime+.05)return this._lastVideoTime=e.currentTime,this._sawProgress=!0,this._stallStrikes=0,this._streamLoaded||(this._streamLoaded=!0),void(++this._healthyTicks>=5&&(this._reloadAttempts=0,this._reloadCooldownUntil=0));if(this._healthyTicks=0,!this._sawProgress||e.paused)return;if(++this._stallStrikes<8)return;this._stallStrikes=0;const i=Date.now();if(!(i<this._reloadCooldownUntil)){if(this._reloadAttempts>=3)return this._reloadCooldownUntil=i+6e4,void(this._reloadAttempts=0);this._reloadAttempts+=1,this._reloadStream()}}_reloadStream(){this._stallStrikes=0,this._sawProgress=!1,this._lastVideoTime=0,this._streamLoaded=!1,this._streamEpoch+=1}_onStreamLoad(){this._streamLoaded||(this._streamLoaded=!0)}render(){if(!this.hass||!this._config)return D`<ha-card><div class="header"><span class="device-name">Nanit</span></div></ha-card>`;const t=this._entities(),e=this._isCameraOn(t);!e&&this._streamLoaded&&(this._streamLoaded=!1,this._sawProgress=!1,this._stallStrikes=0,this._healthyTicks=0,this._reloadAttempts=0,this._reloadCooldownUntil=0,this._stopStreamWatch());const i=t.camera?function(t,e){const i=t.states[e];return i?(i.attributes.friendly_name??"Nanit").replace(/ Camera$/i,""):"Nanit"}(this.hass,t.camera):"Nanit";return D`
      <ha-card>
    ...
    `}_renderHeader(t,e,i){const s=!this._config.hide_connectivity_status&&(bt(this.hass,e.wifi_ssid)||bt(this.hass,e.wifi_signal)||bt(this.hass,e.wifi_frequency)),r=!this._config.hide_baby_name,n=e.power&&!this._config.hide_power_button;return r||!i||s||n?D`
      <div class="header">
        ${r?D`
            ...
            `:V}
        ${i?V:D`<span class="camera-off-label">Camera Off</span>`}
    ...
    `}_renderSoundMachine(t){const e=this.hass.states[t],i="playing"===e?.state,s=e?.attributes.source??"",r=e?.attributes.source_list??[],n=e?.attributes.volume_level??0,a=Math.round(100*n);return D`
      <div class="control-section control-section-sound">
        ...
        <div class="control-row">
          ...
          ${i?D`<span class="track-name">${this._formatSourceName(s)}</span>`:V}
          <div class="slider-row">
